...
function SetupStatusRow({ status }: { status: RuntimeSetupStatusDto }) {
  ...
  const canRetry = status.key === "sandbox" && status.tone === "danger";
  return (
    <div className="grid gap-2 rounded-lg border border-[rgb(var(--border))] bg-[rgb(var(--background)/0.44)] px-3 py-2">
      <div className="flex min-w-0 items-center gap-2">
        ...
        {canRetry ? (
          <Button type="button" variant="soft" size="sm" disabled={retryBusy} onClick={() => {
            setRetryBusy(true);
            void retrySandboxSetup().catch(() => undefined).finally(() => setRetryBusy(false));
          }}>
