...
import { HOME_SESSION_ID, HOME_SESSION_NAME } from "../../src/session/home-session";
  ...
  return (
    <PageFrame eyebrow="Memory" title="Saved sessions without the sidebar ceremony.">
      ...
      <div className="app-sessions-grid grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <HomeSessionCard onOpen={() => open(HOME_SESSION_ID)} />
...
function HomeSessionCard({ onOpen }: { onOpen: () => void }) {
  return (
    <div
      onClick={onOpen}
      className="app-session-card group flex cursor-pointer flex-col gap-3 rounded-[20px] border border-[rgb(var(--border))] bg-[rgb(var(--panel)/0.72)] p-4 transition hover:-translate-y-0.5 hover:bg-[rgb(var(--panel)/0.95)]"
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-2 text-base">
          <House className="h-4 w-4 text-[rgb(var(--muted-foreground))]" />
          <span>{HOME_SESSION_NAME}</span>
        </div>
      </div>
      <div className="flex items-center justify-between font-mono text-xs text-[rgb(var(--muted-foreground))]">
        <span>Default</span>
