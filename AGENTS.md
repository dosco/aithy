...
- Agent soul fields are stored in `metadata` under `bot.*` keys.
...
- Default soul fields live in `config/config.json`.
- On startup, `loadOrSeedSoul` reads the SQLite soul profile and seeds from `config/config.json` only when no soul fields exist.
- Mutable soul fields are stored in the SQLite `metadata` table under `bot.name`, `bot.description`, `bot.coreNature`, `bot.communicationStyle`, `bot.behaviour`, `bot.negativeBehavior`, and `bot.updatedAt`.
- `renderResponderDescription` builds the responder description from the stored fields.
- Apply soul guidance only through `responderOptions.description`.
